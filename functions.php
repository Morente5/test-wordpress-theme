...
require_once get_theme_file_path() . '/inc/navwalker-yofisio.php';
...
class Yofisio_Comment_Walker extends Walker_Comment {
    ...
    function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
        ...
        <article <?php comment_class(empty( $args['has_children'] ) ? '' :'parent') ?> id="comment-<?php comment_ID() ?>" itemprop="comment" itemscope itemtype="http://schema.org/Comment">
            <figure class="avatar float-left"><?php echo get_avatar( $comment, 64, '', 'Avatar' ); ?></figure>
                    ...
                    <?php
                        if ( get_previous_post_link() ) {
                            previous_post_link(
                                ...
                                _x( '<i class="fa fa-hand-o-left"></i>&nbsp;%title', 'Previous post link', 'understrap' ),
                                true
                            );
                        }
...
add_filter( 'avatar_defaults', 'yofisio_avatar' );
function yofisio_avatar($avatar_defaults) {
    $myavatar = 'https://yofisio.es/wp-content/uploads/yofisio_avatar.png';
    $avatar_defaults[$myavatar] = "Yofisio Avatar";
